...
static PONG_TIME = 500;

static int mole_exists = 0; //number of moles that exist at a certain time
static int mole_generate_time[8]; //when each mole was generated;
static int mole_location; //temporary variable to store where mole is generated
static uint8_t moles; //where there are moles on the board
static Time_t game_time = 0; //general game time variable
static Time_t select_press_time = 0; //when the select button was pressed, for debouncing
static Time_t total_game_time; //total amount of time the game is going on for. Used for timing a game of mole
static uint8_t score = 0; //score variable for mole
static Time_t button_debouncing; //used for debouncing hammer press in mole
static int position = 0; //used in pong
static int direction = 0; //used in pong
static int reversed = 0; //used in pong
